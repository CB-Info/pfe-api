...
describe('OrderController', () => {
  ...
  describe('createOne', () => {
    ...
    it('should handle creation errors', async () => {
      mockOrderService.createOne.mockRejectedValue(
        new Error('Creation failed'),
      );

      await expect(controller.createOne(orderDto)).rejects.toThrow(
        'Creation failed',
      );
    });
  ...
  describe('updateOne', () => {
    ...
    it('should handle update errors', async () => {
      ...
      await expect(
        controller.updateOne({ id: 'order123' }, updateData),
  ...
  describe('deleteOne', () => {
    ...
    it('should handle deletion errors', async () => {
      mockOrderService.deleteOne.mockRejectedValue(
        new Error('Deletion failed'),
      );

      await expect(controller.deleteOne({ id: 'order123' })).rejects.toThrow(
        'Deletion failed',
      );
    });
  ...
  describe('Edge cases and error handling', () => {
    ...
    it('should handle empty body for create', async () => {
      ...
      await expect(controller.createOne({} as OrderDTO)).rejects.toThrow(
        'Validation failed',
      );
    });

    it('should handle null update data', async () => {
      mockOrderService.updateOne.mockRejectedValue(
        new Error('No data provided'),
      );

      await expect(
        controller.updateOne({ id: 'order123' }, null),
