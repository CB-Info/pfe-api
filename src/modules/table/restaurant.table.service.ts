...
export class RestaurantTableService {
  ...
  async createOne(
    restaurantTableData: RestaurantTableDTO,
  ...
  async findOne(id: string): Promise<RestaurantTable> {
    try {
      const response = await this.restaurantTableRepository.findOneBy({
        _id: id,
      });
  ...
  async updateOne(
    id: string,
    restaurantTableData: DataType,
  ): Promise<RestaurantTable> {
    try {
      const isUpdate = await this.restaurantTableRepository.updateOneBy(
        { _id: id },
        restaurantTableData,
      );
